...
int main(void)
{
    ...
    // Program won't work with yes or no, but let's try it a bit( we tried doing with get_string but it needs further functions which are available in another library called string.h) There is the strcmp and also strcasecmp which is case sensitive
